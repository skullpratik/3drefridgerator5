...
export const Experience = forwardRef(({ lighting = "photo_studio_01_1k.hdr", doorType = "solid", isReflective }, ref) => {
  ...
  // We no longer create programmatic lights here. The scene should be lit by the
  // HDR environment only. Remove embedded lights that may exist inside the GLTF
  // so they don't conflict with the HDRI.

  // Fixed exposure (HDRI-driven). If you want to expose this, add a simple control later.
  useEffect(() => {
    if (gl && gl.toneMappingExposure !== undefined) gl.toneMappingExposure = 1;
  }, [gl]);

  // Debug: log which controlled names were discovered and their types
  useEffect(() => {
    // Previous implementation referenced `spotEntities` (removed); keep a safe scene-based
    // debug that lists a few lights/meshes to help troubleshooting without throwing.
    if (!scene) return;
    const entries = [];
    scene.traverse((obj) => {
      if (!obj.name) return;
      if (obj.isLight || obj.isMesh) entries.push({ name: obj.name, type: obj.type });
    });
    if (entries.length > 0) console.info("Scene entries (first 50):", entries.slice(0, 50));
  }, [scene]);

  // Remove embedded lights from the imported GLTF so only the HDR environment lights the scene
  useEffect(() => {
    if (!scene) return;
    const toRemove = [];
    scene.traverse((obj) => {
    ...
    if (toRemove.length > 0) console.info(`Removed ${toRemove.length} embedded lights from GLTF`);
  }, [scene]);
...
  useEffect(() => {
    if (!scene) return;
    // Exclude all meshes under objects named 'Logo' or 'Logo2'
    const excludedLogoNames = ["Logo", "Logo2"];
    scene.traverse((child) => {
      // Check if this mesh or any parent is a logo
      let isLogo = false;
      let obj = child;
      while (obj) {
        if (excludedLogoNames.includes(obj.name)) {
          isLogo = true;
          break;
        }
        obj = obj.parent;
    ...
    <>
      <Suspense fallback={null}>
        <OrbitControls
          ...
          makeDefault
        />
  {/* ContactShadows for simple visible shadow */}
        <ContactShadows
          position={[0, -0.9, 0]}
          opacity={1}
          scale={18}
          ...
          resolution={512}
        />
        {/* Use an HDRI environment for lighting */}
