...
import { Interface as NewVisicoolerInterface } from "./components/NewVisicoolerInterface";
...
import { Experience as NewVisicoolerExperience } from "./components/NewVisicoolerExperience";
...
const models = [
  ...
  { value: "newvisicooler", name: "New Visicooler", img: "/images/visicooler.png" },
...
export default function App() {
  ...
  // Controlled LED state for NewVisicooler
  const [newVisiLedOn, setNewVisiLedOn] = useState(false);
  const [doorType, setDoorType] = useState("solid");
  // Glow material color state for NewVisicooler
  ...
  const handleNewLEDToggle = (visible) => {
    setNewVisiLedOn(visible);
    // Call the imperative API on the experience to update the light
    if (newVisiRef.current && typeof newVisiRef.current.toggleLED === 'function') {
      newVisiRef.current.toggleLED(visible);
    }
  };
  ...
  // NOTE: envSettings removed per request - HDRI environment is used inside experiences
        ...
        {open && (
          <>
            ...
            <Box sx={{ p: 3, height: "100%", overflowY: "auto" }}>
              ...
              {modelType === "newvisicooler" && (
                <NewVisicoolerInterface
                  ledOn={newVisiLedOn}
                  onLEDToggle={handleNewLEDToggle}
                  glowColor={newVisiGlowColor}
                  onGlowColorChange={color => setNewVisiGlowColor(color)}
                  onFridgeColorChange={setFridgeColor}
                  fridgeColor={fridgeColor}
      ...
      <Box sx={{ flex: 1, position: "relative" }}>
        ...
        >
          ...
          <CanvasContent
            ...
            louverTextureUrl={louverTextureUrl}
            isReflective={isReflective}
            newVisiGlowColor={newVisiGlowColor}
            insideStripTexture={insideStripTexture}
            fridgeColor={fridgeColor}
            handleColor={handleColor}
            sidePanelLeftTexture={sidePanelLeftTexture}
